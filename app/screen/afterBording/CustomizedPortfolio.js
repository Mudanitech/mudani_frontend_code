  ...
  getPlans = () => {
    ...
    getAPI(`getQuestionnaire`, null).then(response => {
      if (response.status == 200) {
        ...
        console.log(
          'Response Dat' + JSON.stringify(response.data[0].questions),
        );
      } else {
        this.setState({loadingSpinner: false});
      ...
      <View>
        <TouchableOpacity
          ...
          <Text
            style={
              item.status != 1
                ? [styles.yearText, {color: colors.blue, width: width / 1.5}]
                : [styles.yearText, {width: width / 1.5}]
            }>
            {item.answer}
          </Text>
        </TouchableOpacity>
      ...
      <SafeAreaView style={{flex: 1}}>
        <HeaderWithBackWhite
          ...
          onActionLeft={() =>
            this.setState({
              stepIndex:
                this.state.stepIndex == 0 ? this.goBack() : this.previousStep(),
            })
          }
        />
